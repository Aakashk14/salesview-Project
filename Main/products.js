...
destination:async function(req,file,cb){
    console.log("in multerr",req.body)
...
res.send("Wrong")
    ...
    let item_id = await fetch_item_id(req.session.myid)
    console.log("writing file",item_id,req.session.myid)
    
    fs.writeFileSync(`./Storage/users/${req.session.myid}/${item_id}/${item_id}.txt`,req.body.desc,err=>{
        if(err) console.log(err)
    })
       await db.add(req.session.myid,req.body.name,`${item_id}.txt`,1,identity,item_id,req.file.filename)
...
router.get("/product/:productid",async(req,res)=>{
    console.log("executinggg product",req.params)

    result = await db.single_item(req.session.myid,req.params.productid);
    console.log("got result",result)
    
    let url = `http://salesview-project.ap-1.evennode.com/item/${req.session.myid}/${result.items[0].identity}`
