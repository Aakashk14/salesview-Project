...
router.get("/image/:id/:item_id/:image",(req,res)=>{
    console.log("aaaa")
    if(req.session.myid==req.params.id){
        let img_url = `./Storage/users/${req.params.id}/${req.params.item_id}/${req.params.image}`
        console.log("aaaa",img_url)
        res.sendFile(img_url,{root:"./"})
    }
})
...
router.post("/payment/:id/:productid",async(req,res)=>{
    ...
    await orders.purchase_create(req.params.id,order_id,req.session.userid,"New",0,req.body.street,req.body.city,req.body.payment)
    msg="Thanks For Purchase, this is Automated Message! Message here for any support"
    db.update_chatB(req.session.myid,req.params.id,msg,false)
    db.update_chatS(req.params.id,req.session.myid,msg,true)
